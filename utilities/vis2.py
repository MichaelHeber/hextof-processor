...
def orthoslices2_3D(data, normalize=True, continuous_update=False, **kwargs):
    if not isnotebook:
    ...
    w_step = widgets.IntSlider(value=1, min=1, max=e_range -1, step=1, description='Step size',
                               disabled=False)
    ...
    ui_pos = widgets.HBox([w_step, w_e, w_kx, w_ky])
    ...
    img_ax = fig.add_axes([.03, .5, .4, .4], xticklabels=[], yticklabels=[])
    img_ax.xaxis.set_major_locator(plt.LinearLocator(5))
    img_ax.yaxis.set_major_locator(plt.LinearLocator(5))

    xproj_ax = fig.add_axes([.03, .2, .4, .28], xticklabels=[], yticklabels=[])
    #xproj_ax.set_xlabel('$k_x$')
    xproj_ax.xaxis.set_major_locator(plt.LinearLocator(5))

    yproj_ax = fig.add_axes([.45, .5, .28, .4], xticklabels=[], yticklabels=[])
    #yproj_ax.yaxis.set_label_position("right")
    #yproj_ax.set_ylabel('$k_y$')
    yproj_ax.yaxis.set_major_locator(plt.LinearLocator(5))
    
    edc_ax = fig.add_axes([.45, 0.05, .47, .43], xticklabels=[], yticklabels=[])
    edc_ax.tick_params(direction="in", bottom=True, top=True, left=True, right=True, which='both')
    edc_ax.yaxis.tick_right()
    edc_ax.yaxis.set_label_position("right")
    edc_ax.xaxis.set_major_locator(plt.LinearLocator(5))
    edc_ax.yaxis.set_major_locator(plt.LinearLocator(5))
    
    mdc_xproj_ax = fig.add_axes([.03, 0.05, .4, .13], xticklabels=[], yticklabels=[])
    #mdc_xproj_ax.
    mdc_xproj_ax.set_xlabel('$k_x$')
    mdc_xproj_ax.xaxis.set_major_locator(plt.LinearLocator(5))
    mdc_xproj_ax.yaxis.set_major_locator(plt.LinearLocator(5))
    
    mdc_yproj_ax = fig.add_axes([.75, .5, .17, .4], xticklabels=[], yticklabels=[])
    mdc_yproj_ax.yaxis.set_label_position('right')
    mdc_yproj_ax.set_ylabel('$k_y$')
    mdc_yproj_ax.yaxis.tick_right()
    mdc_yproj_ax.xaxis.tick_top()
    mdc_yproj_ax.xaxis.set_major_locator(plt.LinearLocator(5))
    mdc_yproj_ax.yaxis.set_major_locator(plt.LinearLocator(5))

    for ax in [img_ax, yproj_ax, xproj_ax, edc_ax]:  # ,cbar_ax]:
    ...
    edc_img = norm_img(data[:,data.shape[1] // 2,data.shape[2] // 2])
    ...
    def update(step,e, kx, ky, clim, cmap, binning, interpolate, grid, trackers, trackerscol):
        
        if normalize:
            e_img = norm_img(data[e:e+step, :, :][::binning, ::binning])
            y_img = norm_img(data[:, ky, :][::binning, ::binning])
            x_img = norm_img(data[:, :, kx][::binning, ::binning])
            mdcx_img= norm_img(data[e, ky, :][::binning])
            mdcy_img= norm_img(data[e, :, kx][::binning])
            edc_img= norm_img(data[:, ky, kx][::binning])
        else:
            e_img = data[e:e+step, :, :][::binning, ::binning]
        ...
        for axis, plot, img in zip([img_ax, yproj_ax, xproj_ax], [e_plot, x_plot, y_plot], [e_img, x_img.T, y_img]):
            ...
            plot.set_data(img)
            #plot.set_clim(tuple(ti*step/2 for ti in clim))
            plot.set_clim(clim)
            plot.set_cmap(cmap)
            axis.grid(grid)
        ...
        edc_ax.clear() 
        mdc_yproj_ax.clear()
        mdc_xproj_ax.plot(mdcx_img)
        edc_ax.plot(edc_img)
        rot=transforms.Affine2D().rotate_deg(270)
        base = pyplot.gca().transData
        mdc_yproj_ax.plot(mdcy_img, transform=rot+base)
        
    interactive_plot = interactive_output(update, {'step':w_step,
                                                    'e': w_e,
                                                   'kx': w_kx,
                                                   'ky': w_ky,
                                                   'clim': w_clim,
